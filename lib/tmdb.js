...
export async function getSeasonDetails(tvId, seasonNumber) {
  ...
  try {
    ...
    console.error("Error Fetch Season:", error);
    return null;
  }
...
export async function getEventMovies(eventType) {
  try {
    let url = '';
    
    switch(eventType) {
      case 'IMLEK':
        url = `${BASE_URL}/discover/movie?with_genres=28,12&with_original_language=zh&sort_by=popularity.desc&language=id-ID`;
        break;
      
      case 'RAMADAN':
        url = `${BASE_URL}/discover/movie?with_genres=18,10751&with_keywords=207928,9715&sort_by=popularity.desc&language=id-ID`;
        break;
      
      case 'NYEPI':
        url = `${BASE_URL}/discover/movie?with_genres=99,18&with_keywords=10683,9840&sort_by=vote_average.desc&vote_count.gte=100&language=id-ID`;
        break;
      
      case 'NATAL':
        url = `${BASE_URL}/discover/movie?with_genres=10751,35&with_keywords=207317&sort_by=popularity.desc&language=id-ID`;
        break;
      
      default:
        url = `${BASE_URL}/discover/movie?with_genres=18&sort_by=popularity.desc&language=id-ID`;
    }
